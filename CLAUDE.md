...
仓库可以 clone 到任意目录，通过根目录的 `./setup` 安装到 `~/.claude/skills/`。`setup` 只管理本项目自己的条目，不应修改其他项目的 skill。
...
- **ships** — 提交代码并推送，可选带版本号（`/ships v0.1.3`）创建 tag 触发部署

## Skills 之间的协作关系

`issue2task` → `plantask` → 开发者实现 → `checktask` 验收 → `ships` 提交推送
...
- skill 不应自行 git commit，提交由用户或 `/ships` 统一处理
